...
function bindEvent(){
    ...
    $('.studnettablebody').on('click','.activebtn', function(){
        ...
        if($(this).hasClass('edit')){
            ...
            $('#submit').click(function(e){
                e.preventDefault();
                //判断数据是否合法
                var changeData = changeDataType('#editmsg', 'Doris_1602766368421')
        ...
        if($(this).hasClass('del')){
            // console.log(typeof trMsg.sNo);
            var comfirmS = comfirm('你要删除' + trMsg.sNo + trMsg.name + '的同学吗？')
    ...
    $('#assmsgbtn').click(function(e){
        var truelyData = changeDataType('#addmsg', 'Doris_1602766368421')
...
$('.nav').on('click', 'dd', function(e){
    ...
    $('#' + $(e.target).attr('data-id')).addClass('show').siblings().removeClass('show');
})
